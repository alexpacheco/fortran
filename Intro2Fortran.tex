...
%\begin{frame}<0>[fragile]{Array Variables}
%  \begin{itemize}
...
\begin{frame}[fragile,allowframebreaks]{Do Construct: Nested}
  ...
  \begin{columns}[t]
    \column{5.5cm}
    \lstinputlisting[language={[90]Fortran},basicstyle=\fontsize{5}{6}\selectfont\ttfamily]{./demo/nested.f90}
    \column{5.5cm}
    \begin{Verbatim}[fontsize=\fontsize{5}{6}\selectfont,formatcom=\color{indigo}]
...
%\begin{frame}<0>[fragile,allowframebreaks]{Namelist}
%  \begin{itemize}
...
%        \item only static objects may be part of a namelist; i.e. dynamically allocated arrays, pointers and the like are not permitted
%      \end{itemize}
...
%%      \end{lstlisting}
%      \framebreak
%     \item Quantum Espresso package uses namelist to read input. 
...
%        \item In the previous example, the \lstfortran{subroutine verlet} is an external procedure and can be called by any program unit with the program.
   ...
   \end{itemize}
%      \framebreak
%    \lstinputlisting[language={[90]Fortran},basicstyle=\fontsize{4}{5}\selectfont\ttfamily]{./demo/sum.f90}
%    \framebreak
...
\begin{frame}[fragile]{Sum of Two numbers}
  \begin{columns}
    \column{5cm}
    \lstinputlisting[language={[90]Fortran},basicstyle=\small\selectfont\ttfamily]{./demo/sum.f90}
    \vspace{1.5cm}
    Subroutine Call \\
    \lstinline[language={[90]Fortran},basicstyle=\small\selectfont\ttfamily]|call calc(x,y,z)|
    \column{5cm}
    \lstinputlisting[language={[90]Fortran},basicstyle=\small\selectfont\ttfamily]{./demo/sumf.f90}
    \vspace{2cm}
    Function Call \\
    \lstinline[language={[90]Fortran},basicstyle=\small\selectfont\ttfamily]|z = calc(x,y)|
  \end{columns}
\end{frame}
...
%  \begin{columns}[t]
%    \column{6cm}
%      \lstinputlisting[language={[90]Fortran},basicstyle=\fontsize{4.5}{5}\selectfont\ttfamily]{./Exercise/Solution/factorial.f90}
%    \column{5cm}
...
%    \column{4.5cm}
%    \begin{eblock}{Example}
%      \lstinputlisting[language={[90]Fortran},basicstyle=\fontsize{4}{5}\selectfont\ttfamily,firstline=393,lastline=413]{./MD/md-v3.f90}
%    \end{eblock}
%  \end{columns}
%\end{frame}
%
...
%        \end{lstlisting}
  ...
  \framebreak
  \lstinputlisting[language={[90]Fortran},basicstyle=\fontsize{5}{6}\selectfont\ttfamily,firstline=1,lastline=20]{./MD/md-v3.f90}
  \begin{lstlisting}[language={[90]Fortran},basicstyle=\fontsize{5}{6}\selectfont\ttfamily,mathescape]
...
\begin{frame}{Additional Exercises: SAXPY}
  \begin{itemize}
    \item SAXPY is a common operation in computations with vector processors included as part of the BLAS routines
    \item[] $y\leftarrow \alpha x + y$
%    \item SAXPY is a combination of scalar multiplication and vector addition
...
\begin{frame}[allowframebreaks]{Additional Exercises: Matrix Multiplication}
  \begin{itemize}
    \item Most Computational code involve matrix operations such as matrix multiplication.
    \item Consider a matrix {\bf C} which is a product of two matrices {\bf A} and {\bf B}:
    \item[] Element {\it i,j} of {\bf C} is the dot product of the $i^{th}$ row of {\bf A} and $j^{th}$ column of {\bf B}
...
\begin{frame}[fragile]{Additional Exercises: Molecular Dynamics}
  \begin{itemize}
    ...
    \item Code can be obtained from \url{https://github.com/alexpacheco/moleculardynamics}
    \item \lstinline|md-orig.f90| is the original code that you should begin working. 
    \item There is no ``correct solution'', however there are multiple solutions \lstinline|md-v{1-5}.f90| based on various concepts presented.
    \item It’s entirely up to you to decide which solution you want to arrive at.
    \item Compare the results of your edited code with that obtained from \lstinline|md-orig.f90|. If the results are not the same, debug your code.
  \end{itemize}
\end{frame}
